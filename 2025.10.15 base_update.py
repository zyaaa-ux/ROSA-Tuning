...
def _rosa_query_sam_exact(z_btm: torch.Tensor, K: int, impl: str = "sam_cpu", pool_getter=_get_rosa_pool) -> torch.Tensor:
    ...
    z_np = to_c_np(z_btm, dtype=_np.int32)
...
def _lcg_route_worker_index_level_sam(args):
    ...
    if _NUMBA_OK:
        ...
        ycf = _lcg_index_level_seq_nb(z, int(K), cand, mask)
        out = []
        for i in range(T):
...
class MultiRouteLCGFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_v: torch.Tensor):
        ...
        z_btm_np = np.asarray(ctx.z_host_pinned, order="C")
        ...
        def _wait_and_lcg():
            _wait_event(ev)
            cand_np = np.asarray(cand_host, order="C")
        ...
        with torch.no_grad():
            ...
            S = torch.einsum("btd,mkd->bmtk", g_no_scale, E_stack_f).to(device)

            y_old_bmt = y_idx.permute(0, 2, 1)
            y_old_exp = y_old_bmt.unsqueeze(-1).expand(-1, -1, -1, topk)

        y_cf_bmtk_np = fut_cf.result()
        with torch.no_grad():
            y_cf_bmtk = torch.from_numpy(y_cf_bmtk_np).to(device=device, dtype=torch.long)
            skip_mask = (y_cf_bmtk == -2)
            pad_mask  = (y_cf_bmtk == -1)
            idx_new   = torch.where(pad_mask, torch.zeros_like(y_cf_bmtk), y_cf_bmtk + 1)
...
def patch_qwen3_with_multiroute_rosa(model: Qwen3ForCausalLM):
    ...
    query_impl_env = os.environ.get("ROSA_QUERY_IMPL", "").lower()
        ...
        def _forward_with_multiroute_rosa(self: Qwen3DecoderLayer,
            ...
            for head in self.rosa_wlm_list:
                if u.dtype is not head.weight.dtype:
                    ...
                    logits_m = F.linear(u, head.weight, None)
            ...
            z_host = torch.empty_like(z_i32, device='cpu', pin_memory=True)
            ...
            def _wait_and_rosa():
                ...
                if _NUMBA_OK and ('_rosa_batch_btm_nb' in globals()):
                    return _rosa_batch_btm_nb(z_np, int(K))
                else:
                    return _rosa_worker_batch_btm_sam_exact(z_np, int(K))
...
def main():
    ...
    if is_main_process():
        ...
        with open(os.path.join(OUTPUT_DIR, "run_meta.json"), "w", encoding="utf-8") as f:
            json.dump(meta, f, ensure_ascii=False, indent=2)
        print(f"meta saved at {os.path.join(OUTPUT_DIR, 'run_meta.json')}")
