...
def patch_qwen3_with_multiroute_rosa(model: Qwen3ForCausalLM):
    ...
    if PER_ROUTE_DIM is None:
        assert hidden_size % M == 0, (
            f"hidden_size={hidden_size} must be divisible by ROSA_NUM_ROUTES={M}; "
            "or explicitly set ROSA_PER_ROUTE_DIM."
        )
        d = hidden_size // M
    else:
        d = int(PER_ROUTE_DIM)
        assert d * M == hidden_size, (
            f"ROSA_PER_ROUTE_DIM * ROSA_NUM_ROUTES must equal hidden_size, "
            f"currently {d} * {M} != {hidden_size}"
        )
...
def build_optimizer_params(model):
    wlm_params, emb_params, gate_params, backbone_params = [], [], [], []
