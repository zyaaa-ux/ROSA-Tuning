...
if "NUMBA_NUM_THREADS" not in _os.environ:
    try:
        n_aff = len(os.sched_getaffinity(0))
    except Exception:
        n_aff = os.cpu_count() or 1
    cap = int(_os.environ.get("ROSA_NUMBA_THREADS", 24))
    _os.environ["NUMBA_NUM_THREADS"] = str(max(1, min(n_aff, cap)))
...
ROSA_K_CONTRAST_USE_V_PROB = int(os.environ.get("ROSA_K_CONTRAST_USE_V_PROB", 1))
...
STAGE_A_TOKENS          = 1500000000
STAGE_A_LR_ROSA         = 3e-4
STAGE_A_MIN_LR_ROSA: float = 5e-5
STAGE_A_MIN_LR_GATE: float = 5e-5
STAGE_A_MIN_LR_BACKBONE: float = 0.0
STAGE_A_WEIGHT_DECAY    = 0.01
STAGE_A_WARMUP_STEPS    = 20
SAVE_STEPS_STAGE_A      = 100
EVAL_STEPS_STAGE_A      = 1000000

STAGE_B_LR_ROSA         = 1e-4
STAGE_B_LR_BACKBONE     = 5e-6
STAGE_B_MIN_LR_ROSA: float = 5e-6
STAGE_B_MIN_LR_BACKBONE: float = 1e-6
...
def _to_p1_i16_np(x_i32: _np.ndarray) -> _np.ndarray:
    y = x_i32.astype(_np.int32) + 1 - 32768
    _np.clip(y, -32768, 32767, out=y)
    return y.astype(_np.int16, copy=False)

def p1_to_raw_i32_torch(x_p1: torch.Tensor) -> torch.Tensor:
    return x_p1.to(torch.int32).add_(32768).sub_(1)
...
if _NUMBA_OK:
    ...
    def _k_rle_batch_btr_nb(z_btr: _np.ndarray):
        B, T, R = z_btr.shape
        run_start_brt = _np.empty((B, R, T), dtype=_np.int32)
        run_sym_brt   = _np.empty((B, R, T), dtype=_np.int32)
...
if _NUMBA_OK:
    ...
    def _compute_k_runcap_brt_nb(k_run_start_brt: _np.ndarray,
        ...
        rcap = _np.empty((B, R, T), dtype=_np.int32)
        ...
        for br in _nb.prange(B * R):
            ...
            for t in range(T):
                while (curr + 1) < clen and rs[curr + 1] <= t:
                    curr += 1
                rcap[b, r, t] = curr
        return rcap
...
if _NUMBA_OK:
    ...
    def _q_prefix_scan_bits_nb(
        ...
        dest_time_brt  = _np.empty((B, R, T), dtype=_np.int32)
        q_run_id_brt   = _np.empty((B, R, T), dtype=_np.int32)
        rq_len_br      = _np.zeros((B, R),    dtype=_np.int32)
        r_cf_run_brrjb = _np.empty((B, R, T, Mbits, 2), dtype=_np.int32)

        flip_lut = _np.empty((K, Mbits, 2), dtype=_np.int32)
        for a in range(K):
            for j in range(Mbits):
                maskj = 1 << j
                flip_lut[a, j, 0] = a & (~maskj)
                flip_lut[a, j, 1] = a |  maskj
        ...
        for br in _nb.prange(B * R):
            ...
            def _extend_to(cap_t):
                nonlocal r_ext
                while r_ext < cap_t and (r_ext + 1) < clen:
                    r_ext += 1
                    _sam_extend_nb(next_arr, link, length, e, last_arr, size, int(c[r_ext]), r_ext)
            ...
            for t in range(T):
                if cur_run < rq and t == run_beg[cur_run]:
                    ...
                    for j in range(Mbits):
                        r_cf_run_brrjb[b, r, cur_run, j, 0] = -1
                        r_cf_run_brrjb[b, r, cur_run, j, 1] = -1

                    for j in range(Mbits):
                        for bit in (0, 1):
                            a_mod = flip_lut[base_a, j, bit]
...
def _rosa_binary_pipeline_cpu(
    ...
    use_u8_transfer = (K <= 256)

    if use_u8_transfer:
        q_host_u8 = _PINNED_POOL.get("q_cat_host_u8", (B, T, R), dtype=torch.uint8)
        k_host_u8 = _PINNED_POOL.get("k_cat_host_u8", (B, T, R), dtype=torch.uint8)
        q_host_i32 = None
        k_host_i32 = None
    else:
        q_host_i32 = _PINNED_POOL.get("q_cat_host", (B, T, R), dtype=torch.int32)
        k_host_i32 = _PINNED_POOL.get("k_cat_host", (B, T, R), dtype=torch.int32)
        q_host_u8 = None
        k_host_u8 = None
    ...
    copy_stream = torch.cuda.Stream(device=q_cat_btr_torch.device)
    copy_stream.wait_stream(torch.cuda.current_stream(device=q_cat_btr_torch.device))

    with torch.cuda.stream(copy_stream):
        if use_u8_transfer:
            q_host_u8.copy_(q_cat_btr_torch.to(torch.uint8), non_blocking=True)
            k_host_u8.copy_(k_cat_btr_torch.to(torch.uint8), non_blocking=True)
        else:
            q_host_i32.copy_(q_cat_btr_torch, non_blocking=True)
            k_host_i32.copy_(k_cat_btr_torch, non_blocking=True)
        ev_copy_done = torch.cuda.Event()
        ev_copy_done.record(copy_stream)

    def _wait_and_run():
        _wait_event(ev_copy_done)

        if use_u8_transfer:
            q_np = _np.asarray(q_host_u8)
            k_np = _np.asarray(k_host_u8)
            q_np = q_np.astype(_np.int32, copy=True)
            k_np = k_np.astype(_np.int32, copy=True)
        else:
            q_np = _np.asarray(q_host_i32)
            k_np = _np.asarray(k_host_i32)

        k_runstart_np, k_runsym_np, k_clen_np = _k_rle_batch_btr_nb(k_np)
        k_runcap_np = _compute_k_runcap_brt_nb(k_runstart_np, k_clen_np, int(T))
        dest_np, q_run_id_np, _rq_len, r_cf_np = _q_prefix_scan_bits_nb(
            q_np, k_runsym_np, k_runstart_np, k_clen_np, k_runcap_np, int(Mbits), int(K)
        )

        if use_i16:
            _np.copyto(_np.asarray(k_run_start_cpu), _to_p1_i16_np(k_runstart_np), casting="no")
            _np.copyto(_np.asarray(dest_time_cpu),   _to_p1_i16_np(dest_np),       casting="no")
            _np.copyto(_np.asarray(q_run_id_cpu),    _to_p1_i16_np(q_run_id_np),   casting="no")
            _np.copyto(_np.asarray(r_cf_bit_cpu),    _to_p1_i16_np(r_cf_np),       casting="no")
        else:
...
class MultiRouteBinaryROSAFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_y: torch.Tensor):
        ...
        v_field_k_32   = v_prob_brtm_32 if (ROSA_K_CONTRAST_USE_V_PROB == 1) else v_bits_brtm_32
...
def patch_qwen3_with_multiroute_rosa(model: Qwen3ForCausalLM):
    ...
    for li, layer in enumerate(model.model.layers):
        ...
        with torch.no_grad():
            eye = torch.eye(C, dtype=base_dtype, device=base_device)
            layer.rosa_out.weight.copy_(eye)
        ...
        def _forward_qkv_brosa(
            ...
            **kwargs
            ...
            C = self.rosa_q_proj.weight.shape[1]
            assert Cx == C
            ...
            BITS_PER_ROUTE = self.rosa_bits_per_route
            R = self.rosa_num_routes
            def _pack_bits_btC_to_btr(bits_btC: torch.Tensor, Mbits: int) -> torch.Tensor:
                B0, T0, C0 = bits_btC.shape
                assert C0 % Mbits == 0
                Rloc = C0 // Mbits
                x = bits_btC.view(B0, T0, Rloc, Mbits).to(torch.int32)
                out = torch.zeros((B0, T0, Rloc), dtype=torch.int32, device=bits_btC.device)
                for j in range(Mbits):
                    out |= ((x[..., j] & 1) << j)
                return out

            def _unpack_btr_to_bits_brtm(cat_btr: torch.Tensor, Mbits: int) -> torch.Tensor:
                B0, T0, Rloc = cat_btr.shape
                x = cat_btr.to(torch.int32)
                bits = []
                for j in range(Mbits):
                    bits.append(((x >> j) & 1))
            ...
            if k_run_start_cpu.dtype == torch.int16:
                k_run_start_brt = p1_to_raw_i32_torch(k_run_start_brt)
                dest_time_brt   = p1_to_raw_i32_torch(dest_time_brt)
                q_run_id_brt    = p1_to_raw_i32_torch(q_run_id_brt)
                r_cf_run_brrjb  = p1_to_raw_i32_torch(r_cf_run_brrjb)

            dest_clamp = dest_time_brt.clamp(0, T0 - 1).to(torch.long)
            valid_dest_brt = (dest_time_brt >= 0)
            ...
            b_bits_btc = v_bits_at_dest_brtm.permute(0, 2, 1, 3).contiguous().view(B0, T0, C)

            delta = (self.rosa_e1 - self.rosa_e0)
            y_valid = self.rosa_e0.view(1, 1, C) + delta.view(1, 1, C) * b_bits_btc.to(delta.dtype)

            valid_mask_btc = valid_dest_brt.permute(0, 2, 1).unsqueeze(-1).expand(B0, T0, R, BITS_PER_ROUTE).reshape(B0, T0, C)
            y_final = y_valid * valid_mask_btc.to(y_valid.dtype)

            pos_mask_cpu = None
            if LCG_POS_SUBSAMPLE < 1.0:
                mask = (torch.rand((B0, T0), device=hidden_states.device) < LCG_POS_SUBSAMPLE)
...
def build_model_and_tokenizer() -> Tuple[Qwen3ForCausalLM, AutoTokenizer]:
    ...
    def _torch_load_weights_only(path: str):
        try:
    ...
    def _load_hf_checkpoint_state_dict(model_dir: str) -> Dict[str, torch.Tensor]:
        import os, json
        ...
        if not os.path.isdir(model_dir):
            if model_dir.endswith(".safetensors"):
                ...
                obj = _torch_load_weights_only(model_dir)
                if isinstance(obj, dict) and "state_dict" in obj:
                    obj = obj["state_dict"]
                if not isinstance(obj, dict):
                    raise ValueError(f"Unexpected checkpoint object type: {type(obj)} from {model_dir}")
                for k, v in obj.items():
                    if torch.is_tensor(v):
                        state_dict[k] = v
                return state_dict

        st_idx = os.path.join(model_dir, "model.safetensors.index.json")
        if os.path.isfile(st_idx):
            from safetensors import safe_open
            with open(st_idx, "r", encoding="utf-8") as f:
                j = json.load(f)
            weight_map = j.get("weight_map", {})
            uniq = sorted(set(os.path.join(model_dir, v) for v in weight_map.values()))
            for fp in uniq:
        ...
        st_files = [fn for fn in sorted(os.listdir(model_dir)) if fn.endswith(".safetensors")]
        if st_files:
            from safetensors import safe_open
            for fn in st_files:
                if not (fn.startswith("model") or fn.startswith("pytorch_model")):
                    continue
        ...
        if os.path.isfile(bin_idx):
            with open(bin_idx, "r", encoding="utf-8") as f:
                j = json.load(f)
            weight_map = j.get("weight_map", {})
            uniq = sorted(set(os.path.join(model_dir, v) for v in weight_map.values()))
            for fp in uniq:
                obj = _torch_load_weights_only(fp)
                if isinstance(obj, dict) and "state_dict" in obj:
                    obj = obj["state_dict"]
                if not isinstance(obj, dict):
                    continue
        ...
        binpt_files = [fn for fn in sorted(os.listdir(model_dir)) if fn.endswith(".bin") or fn.endswith(".pt")]

        def _is_model_weight_file(fn: str) -> bool:
            lower = fn.lower()
            if any(x in lower for x in ["training_args", "optimizer", "scheduler", "rng_state", "trainer_state"]):
                return False
            if lower.startswith("pytorch_model") or lower.startswith("model"):
                return True
            return False

        for fn in binpt_files:
            if not _is_model_weight_file(fn):
                continue
        ...
        if not state_dict:
            raise FileNotFoundError(
                ...
                f"Expected model.safetensors / model.safetensors.index.json / pytorch_model*.bin"
            )
        return state_dict

    if MODEL_DIR is not None:
        try:
            state_dict = _load_hf_checkpoint_state_dict(MODEL_DIR)

            missing, unexpected = _fill_and_tie_tied_lm_for_training(
                model, state_dict, tie=bool(TIE)
            )
            if len(unexpected) > 0:
                print(f"[warn] Unexpected keys when loading MODEL_DIR: {len(unexpected)} (first 10) -> {unexpected[:10]}")
            if len(missing) > 0:
                print(f"[warn] Missing keys when loading MODEL_DIR: {len(missing)} (first 10) -> {missing[:10]}")
        except Exception as e:
            print(f"[error] Loading MODEL_DIR failed: {e}")
...
def build_optimizer_params(model):
    ...
    if gate_nodecay:
        groups.append({
            "name": "gate_nodecay",
            "params": gate_nodecay,
            "lr": STAGE_A_LR_ROSA if inject_mode == "pre_attn" else 0.0,
            "weight_decay": 0.0
        })
    if backbone_decay:
        groups.append({
...
class CosineWithWarmupMinLRScheduler(_LRScheduler):
    def __init__(self, optimizer, num_warmup_steps: int, num_training_steps: int,
                 min_lrs: list[float], last_epoch: int = -1):
        self.num_warmup_steps = int(max(0, num_warmup_steps))
        self.num_training_steps = int(max(1, num_training_steps))
    ...
    def get_lr(self):
        ...
        if step < self.num_warmup_steps:
            w = float(step) / float(max(1, self.num_warmup_steps))
            return [min_lr + (base_lr - min_lr) * w
                    for base_lr, min_lr in zip(self.base_lrs, self.min_lrs)]
        progress = float(step - self.num_warmup_steps) / float(max(1, self.num_training_steps - self.num_warmup_steps))
        cos_decay = 0.5 * (1.0 + math.cos(math.pi * min(1.0, progress)))
        return [min_lr + (base_lr - min_lr) * cos_decay
                for base_lr, min_lr in zip(self.base_lrs, self.min_lrs)]

    def reconfigure(self, num_warmup_steps: int, num_training_steps: int, min_lrs: list[float]):
        self.num_warmup_steps = int(max(0, num_warmup_steps))
        self.num_training_steps = int(max(1, num_training_steps))
        self.min_lrs = list(min_lrs)
...
class TwoStageSwitchCallback(TrainerCallback):
    ...
    def on_step_end(self, args, state, control, model=None, optimizer=None, lr_scheduler=None, **kwargs):
        ...
        for pg in optimizer.param_groups:
            name = pg.get("name", "")
            if name in ("rosa_heads_decay", "emb_nodecay"):
                pg["lr"] = float(STAGE_B_LR_ROSA)
            elif name == "gate_nodecay":
                pg["lr"] = float(STAGE_B_LR_ROSA if inject_mode == "pre_attn" else 0.0)
            elif name in ("backbone_decay", "backbone_nodecay"):
                pg["lr"] = float(STAGE_B_LR_BACKBONE)

        print("[TwoStage] param group LRs updated for Stage-B.")

        if isinstance(lr_scheduler, CosineWithWarmupMinLRScheduler):
            remaining = max(1, int(state.max_steps) - int(state.global_step))
            min_lrs_b = []
            for pg in optimizer.param_groups:
                name = pg.get("name", "")
                if name in ("rosa_heads_decay", "emb_nodecay", "gate_nodecay"):
                    if name == "gate_nodecay" and inject_mode != "pre_attn":
                        min_lrs_b.append(0.0)
                    else:
                        min_lrs_b.append(float(STAGE_B_MIN_LR_ROSA))
                elif name in ("backbone_decay", "backbone_nodecay"):
                    min_lrs_b.append(float(STAGE_B_MIN_LR_BACKBONE))
                else:
                    min_lrs_b.append(0.0)

            lr_scheduler.reconfigure(
                num_warmup_steps=int(STAGE_B_WARMUP_STEPS),
                num_training_steps=int(remaining),
                min_lrs=min_lrs_b,
            )
            print(f"[TwoStage] LR scheduler -> cosine_with_warmup(min_lr) | warmup={STAGE_B_WARMUP_STEPS}, remaining_steps={remaining}.")
        else:
            print("[TwoStage] WARN: lr_scheduler is not CosineWithWarmupMinLRScheduler; skip reconfigure.")
    ...
    class _Trainer(Trainer):
        ...
        def create_scheduler(
            ...
            **kwargs,
            ...
            def _stage_a_min_lrs():
                mins = []
                inject_mode = os.environ.get("ROSA_INJECT_MODE", globals().get("ROSA_INJECT_MODE", "pre_attn")).lower()
                for pg in self.optimizer.param_groups:
                    name = pg.get("name", "")
                    if name in ("rosa_heads_decay", "emb_nodecay"):
                        mins.append(float(STAGE_A_MIN_LR_ROSA))
                    elif name == "gate_nodecay":
                        mins.append(float(STAGE_A_MIN_LR_GATE if inject_mode == "pre_attn" else 0.0))
                    elif name in ("backbone_decay", "backbone_nodecay"):
                        mins.append(float(STAGE_A_MIN_LR_BACKBONE))
                    else:
                        mins.append(0.0)
                return mins
            ...
            self.lr_scheduler = CosineWithWarmupMinLRScheduler(
                self.optimizer,
                num_warmup_steps=int(STAGE_A_WARMUP_STEPS),
                num_training_steps=int(num_training_steps),
                min_lrs=_stage_a_min_lrs(),
            )
            return self.lr_scheduler
